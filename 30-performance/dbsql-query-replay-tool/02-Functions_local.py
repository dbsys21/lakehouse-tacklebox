...
from pyspark.sql.functions import from_unixtime, lit, col, struct
...
class QueryReplayTest:
    ...
    def store_query_history(self, warehouse_id, start_time_ms=None, end_time_ms=None):
        ...
        while (has_next_page and pages_fetched < MAX_PAGES_PER_RUN):
            ...
            response = requests.get(f"https://{self.host}/api/2.0/sql/history/queries", headers=auth_header, json=request_parameters)
            ...
            query_results = spark.createDataFrame(queries_json)

            query_results = query_results.withColumn(
                    "channel_used",
                    struct(
                        col("channel_used").getItem("dbsql_version").alias("dbsql_version"),
                        col("channel_used").getItem("name").alias("name")
                    )
            )
        ...
        # query_results_clean.show()

    # Check if the table exists
    def table_exists(self, catalog, database, table):
        try:
            sql_query = f"DESCRIBE EXTENDED {catalog}.{database}.{table}"
            result = spark.sql(sql_query).collect()
            # Check if the result contains information about the table
            return len(result) > 0
        except Exception as e:
            # Handle the exception (table not found) and return False
            return False
